  ...
  return (
    <div className="min-h-[100dvh] pb-[calc(7rem+env(safe-area-inset-bottom,0px))]">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 space-y-5">
        <h1 className="font-display text-2xl font-bold text-mk-gold uppercase tracking-widest">{t.stats.title}</h1>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="card text-center">
            ...
            <p className="text-white/50 text-sm mt-1">{t.stats.myAttendance}</p>
            <p className="text-white/30 text-xs">{t.stats.last3Months}</p>
          </div>
          <div className="card text-center">
            <p className="text-5xl font-display font-bold text-white/40">
              {teamRate !== null ? `${teamRate}%` : '—'}
            </p>
            <p className="text-white/50 text-sm mt-1">{t.stats.teamAverage}</p>
            <p className="text-white/30 text-xs">{t.stats.last3Months}</p>
          </div>
        </div>
        ...
        <div className="card">
          <p className="text-white/50 text-sm">
            <span className="text-white font-semibold">{present}</span> {t.stats.presentOutOf}{' '}
            <span className="text-white font-semibold">{total}</span> {t.stats.recordedSessions}
          </p>
        </div>

        <div className="card space-y-2">
          <h2 className="font-display text-sm uppercase tracking-widest text-white/40 mb-2">{t.stats.subscribeCalendar}</h2>
          <p className="text-white/50 text-xs">{t.stats.calendarDescription}</p>
        ...
        {/* Absences */}
        <div className="card space-y-2">
          <h2 className="font-display text-sm uppercase tracking-widest text-white/40 mb-2">{t.stats.absences}</h2>
          <p className="text-white/50 text-xs mb-3">{t.stats.absencesDescription}</p>
          <AbsenceManager />
        </div>

        {/* History */}
        <div className="card space-y-2">
          <h2 className="font-display text-sm uppercase tracking-widest text-white/40 mb-3">{t.stats.history}</h2>
          {records.length === 0 && (
            <p className="text-white/30 text-sm">{t.stats.noAttendance}</p>
          )}
          {records.map((rec) => (
            <div key={rec.id} className="flex items-center gap-3 py-1.5 border-b border-white/5 last:border-0">
              ...
              <span className={`text-xs uppercase tracking-wide ${rec.present ? 'text-green-400' : 'text-red-400'}`}>
                {rec.present ? t.stats.present : t.stats.absent}
              </span>
            </div>
          ))}
        </div>
